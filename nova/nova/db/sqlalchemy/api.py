...
def phy_host_get_all(context):
    query = model_query(context, models.PhyHost, read_deleted="no")
    return query.all()
...
def phy_host_get_all_by_service_id(context, service_id, session=None):
    if not session:
        session = get_session()
    query = model_query(context, models.PhyHost, read_deleted="no", session=session).\
...
def phy_host_get(context, phy_host_id, session=None):
    ...
    result = model_query(context, models.PhyHost, read_deleted="no", session=session).\
...
def phy_pxe_ip_get_all(context):
    query = model_query(context, models.PhyPxeIp, read_deleted="no")
    return query.all()
...
def phy_pxe_ip_create(context, address, server_address, service_id, session=None):
    if not session:
        session = get_session()
    ref = model_query(context, models.PhyPxeIp, read_deleted="no", session=session).\
                     filter_by(address=address).\
                     filter_by(service_id=service_id).\
                     first()
    if not ref:
...
@require_admin_context
def phy_pxe_ip_get(context, id, session=None):
    if not session:
        session = get_session()
    result = session.query(models.PhyPxeIp).\
                     filter_by(id=id).\
                     first()
    return result
...
def phy_pxe_ip_associate(context, phy_host_id):
    ...
    with session.begin():
        ...
        if not ph_ref:
            raise exception.Error(host=phy_host_id)
        ip_ref = session.query(models.PhyPxeIp).\
                         filter_by(deleted=False).\
...
@require_admin_context
def phy_interface_get(context, id, session=None):
    if not session:
        session = get_session()
    result = model_query(context, models.PhyInterface, read_deleted="no", session=session).\
                     filter_by(id=id).\
                     first()
    return result
...
def phy_interface_get_all(context, session=None):
    session = get_session()
    with session.begin():
        query = model_query(context, models.PhyInterface, read_deleted="no", session=session)
        return query.all()


@require_admin_context
def phy_interface_destroy(context, id):
    session = get_session()
    with session.begin():
        session.query(models.PhyInterface).\
                filter_by(id=id).\
                update({'deleted': True,
                        'deleted_at': utils.utcnow(),
                        'updated_at': literal_column('updated_at')})
...
@require_admin_context
def phy_interface_set_vif_uuid(context, id, vif_uuid):
    session = get_session()
    with session.begin():
        ref = session.query(models.PhyInterface).\
                         filter_by(deleted=False).\
                         filter_by(id=id).\
                         with_lockmode('update').\
                         first()
...
def phy_interface_get_by_vif_uuid(context, vif_uuid):
    session = get_session()
    with session.begin():
        result = session.query(models.PhyInterface).\
...
@require_admin_context
def phy_deployment_get(context, id, session=None):
    if not session:
        session = get_session()
    result = session.query(models.PhyDeployment).\
                     filter_by(id=id).\
                     first()
    return result
...
@require_admin_context
def phy_deployment_destroy(context, id):
    session = get_session()
    with session.begin():
        session.query(models.PhyDeployment).\
                filter_by(id=id).\
                update({'deleted': True,
                        'deleted_at': utils.utcnow(),
                        'updated_at': literal_column('updated_at')})
